...
function Donation() {
  ...
  const [displayedFeeType, setDisplayedFeeType] = useState("");
  ...
  const handleGenerateRegister = () => {
    if (selectedYear && selectedFeeType) {
      setDisplayedYear(selectedYear);
      setDisplayedFeeType(selectedFeeType);
      setShowRegister(true);
      const newData = getFilteredData(selectedYear, selectedFeeType);
  ...
  const getFilteredData = (year, feeType, dataArray) => {
    const filteredData = dataArray.filter(
      (row) => row.year === parseInt(year) && row.feeType === feeType
    );
    return filteredData;
  };
    ...
    <div className="p-3">
      ...
      <Accordion defaultActiveKey={["0"]} alwaysOpen>
        <Accordion.Item eventKey="0">
          ...
          <Accordion.Body>
            <div className="input-field d-flex">
              <div className="col-3 me-3 cl_left">
      ...
      {showRegister && <DonateData selectedYear={displayedYear} selectedFeeType={displayedFeeType} />}
      {/* {loading ? <div>Loading...</div> : showRegister && <DonateData selectedYear={displayedYear} selectedFeeType={displayedFeeType} />} */}
    </div>
  );
}
